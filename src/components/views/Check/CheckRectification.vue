<template>
  <el-card class="box-card boundary-C" shadow="never"
    ...
    <div id="dashboard_2" style="height: 80%; width: 100%;"></div>
  </el-card>
</template>
...
export default {
  ...
  methods: {
    drawDashboard() {
      let myChart = this.$echarts.init(document.getElementById('dashboard_2'))
      // 使用刚指定的配置项和数据显示图表。
      ...
      erd.listenTo(document.getElementById("dashboard_2"), element => {
        _this.$nextTick(() => {
          //监听到事件后执行的业务逻辑
          myChart.resize();
        });
